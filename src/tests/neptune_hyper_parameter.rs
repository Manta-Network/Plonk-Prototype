...
use crate::tests::conversion::field_cast;
...
#[cfg(test)]
pub(crate) fn collect_neptune_constants<A>(strength:Strength) -> (Vec<Vec<ArkFr>>, Vec<ArkFr>)
where
    A: Arity<Fr>,
{
    ...
    let mut Ark_mds_matrices = Vec::with_capacity(width);
    for i in 0..width {
        let mut row = Vec::with_capacity(width);
        for j in 0..width {
            let bls_fr = mds_matrices[i][j];
            let ark_fr = field_cast(bls_fr);
            row.push(ark_fr);
        }
        Ark_mds_matrices.push(row)
    }

    let mut Ark_round_constants = Vec::with_capacity(width);
    for i in 0..width {
        let bls_fr = round_constants[i];
        let ark_fr = field_cast(bls_fr);
        Ark_round_constants.push(ark_fr);
    }

    (Ark_mds_matrices, Ark_round_constants)
}
