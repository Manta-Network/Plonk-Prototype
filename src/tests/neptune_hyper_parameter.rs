...
use neptune::poseidon::{HashMode, PoseidonConstants as NeptunePoseidonConstants, Arity, Poseidon as NeptunePoseidon};
...
use crate::tests::conversion::{cast_field, cast_vector, cast_matrix};
...
#[cfg(test)]
pub(crate) fn collect_neptune_constants<A, F>(strength:Strength) -> PoseidonConstants<F>
where
    A: Arity<Fr>,
    F: ArkPrimeField,
{
    ...
    for i in 0..num_sparse_matrix {
        let w_hat = sparse_matrixes[i].w_hat;
        let v_rest = sparse_matrixes[i].v_rest;
...
mod tests {
    ...
    fn convert_neptune_constants_test() {
        collect_neptune_constants::<typenum::U2, ArkFr>(Strength::Standard);
