    ...
    match field {
        1 => {
            for _ in 0..num_constants {
                while {
                    // TODO: Please review this part. May be different from neptune.
...
impl GrainLFSR {
    pub fn new(initial_sequence: VecDeque<bool>, field_size: u16) -> Self {
        assert_eq!(initial_sequence.len(), 80, "Initial Sequence must be 80 bits");
        let mut g = GrainLFSR {
            state: initial_sequence,
            field_size,
        };
        (0..160).for_each(|_| { g.generate_new_bit(); });
